...
def welch_psd(nni=None,
	...
	if mode == 'normal':
		...
		# Output
		return pu.join_tuples(params, figure, meta)
	...
	# Returns frequency parameters and frequency & power series/array; does not create a plot figure nor plot the data
		...
		# Output
		return pu.join_tuples(params, meta), frequencies, np.asarray((powers / 10 ** 6))
	...
	# Returns frequency parameters, PSD plot figure, and frequency & power series/arrays
		...
		# Output
		return pu.join_tuples(params, figure, meta), frequencies, np.asarray((powers / 10 ** 6))
...
def lomb_psd(
	...
	# Returns frequency parameters and frequency & power series/array; does not create a plot figure nor plot the data
		...
		# Complete output
		return pu.join_tuples(params, meta), frequencies, powers
	...
	# Returns frequency parameters, PSD plot figure, and frequency & power series/arrays
		...
		# Complete output
		return pu.join_tuples(params, figure, meta), frequencies, powers
...
# 		params = pu.join_tuples(params, meta)
...
# 		# Complete output
# 		return pu.join_tuples(params, figure)
...
# 		# Output
# 		return pu.join_tuples(params, meta), frequencies, (powers / 10 ** 6)
...
# 		# Complete output
# 		return pu.join_tuples(params, figure, meta), frequencies, (powers / 10 ** 6)
...
def psd_comparison(nni=None,
	...
	if segments is None:
		nn = pu.check_input(nni, rpeaks)
		segments, worked = pu.segmentation(nn, full=False, duration=duration)
	else:
	...
	if not worked:
		...
		for i in range(len(psd_data)):
			output = pu.join_tuples(biosppy.utils.ReturnTuple((psd_data[i]['params'],), ('seg%i' % i,)), output)
		return output
...
def psd_waterfall(nni=None,
	...
	if segments is None:
		nn = pu.check_input(nni, rpeaks)
		y_unit_time = True
		segments, worked = pu.segmentation(nn, duration=duration, full=False)
	else:
		y_unit_time = False
		worked = True
	...
	# If segmentation worked, proceed with plot
		...
		for i in range(len(segment_parameters)):
			output = pu.join_tuples(biosppy.utils.ReturnTuple((segment_parameters[i], ), ('seg%i' % i, )), output)
		return output
...
def frequency_domain(nni=None,
	...
	# Output
	return pu.join_tuples(welch_results, lomb_results)

...
def sdnn_index(nni=None, rpeaks=None, full=True, duration=300, warn=True):
	...
	segments, seg = pu.segmentation(nn,  full=full, duration=duration, warn=warn)
...
def time_domain(nni=None,
	...
	results = pu.join_tuples(results, hr_parameters(nn))
	results = pu.join_tuples(results, nni_differences_parameters(nn))
	results = pu.join_tuples(results, sdnn(nn))
	results = pu.join_tuples(results, sdnn_index(nn))
	results = pu.join_tuples(results, sdann(nn))
	results = pu.join_tuples(results, rmssd(nn))
	results = pu.join_tuples(results, sdsd(nn))
	results = pu.join_tuples(results, nn50(nn))
	results = pu.join_tuples(results, nn20(nn))

	# Compute custom threshold if required
	if threshold is not None and threshold not in [50, 20]:
		results = pu.join_tuples(results, nnXX(nn, threshold=int(threshold)))

	# Compute geometrical parameters
	results = pu.join_tuples(results, geometrical_parameters(nn, plot=plot, show=show, binsize=binsize))
...
if __name__ == "__main__":
	...
	nni = pu.load_sample_nni(series='long')

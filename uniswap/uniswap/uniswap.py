...
class Uniswap:
    ...
    def __init__(
        ...
        if self.version == 1:
            ...
            self.NFT_position_address = _str_to_addr("0xC36442b4a4522E871399CD717aBDD847Ab11FE88")
            self.NFT_position_contract = _load_contract(
                self.w3, abi_name="uniswap-v3/NFT_position", address=self.NFT_position_address
            )
        else:
            raise Exception(
...
#        token_reserve = self.get_ex_token_balance(token)
    ...
    def create_pool_v3(
        self, token1: AddressLike, token2: AddressLike, fee: int, max_eth: Wei
    ) -> HexBytes:
        """Create the pool for liquidity."""
        func_params = [token1, token2, fee]
        function = self.factory_contract.functions.createPool(*func_params)
        tx = self._build_and_send_tx(function)
        time.sleep(1)
    
    def create_and_initialize_pool(
    	self, token1: AddressLike, token2: AddressLike, fee: int, sqrtX96: int
    ) -> HexBytes:
    	function = self.NFT_position_contract.functions.createAndInitializePoolIfNecessary(token1, token2, fee, int("000000000000000000000000000000000000000185b96aabfac7cdc4b3b58fc2", 16))
    	return self._build_and_send_tx(function)
    
    def deposit_liquidity_v3(
    	self, token0: AddressLike, token1: AddressLike, fee: int, tickLower: int, tickUpper: int, amount0Desired: int, amount1Desired: int,  amount0Min: int, amount1Min: int, recipient: AddressLike
    ) -> HexBytes:
        print("Deposit tokens")
        
        self.approve_to_pool(token0, amount0Desired, self.NFT_position_address)
        self.approve_to_pool(token1, amount1Desired, self.NFT_position_address)
        
        mint = self.NFT_position_contract.get_function_by_signature('mint(tuple)')
        p = [_str_to_addr(token0), _str_to_addr(token1), fee, tickLower, tickUpper, amount0Desired, amount1Desired, amount0Min, amount1Min, recipient, self._deadline()+100]
        function = mint(p)
    ...
    def burn_v3(
        ...
        print("Burn", tokenId)
        function = self.NFT_position_contract.function.burn([tokenId])
        return self._build_and_send_tx(function)
    
    def increase_liquidity_v3(
        self, tokenId: int, amount0Desired: int, amount1Desired: int,  amount0Min: int, amount1Min: int
    ) -> HexBytes:
        print("Increase Liquidity")
        ...
        #pool_contract = _load_contract(self.w3, abi_name="uniswap-v3/pool", address=pool_address)
        
        #pool_id = int(pool_address, 0)
        ...
        increase_liquidity_params = [
        	...
        	amount0Min,
        	amount1Min,
        	self._deadline()+1000*60
        ]
        increase_liquidity = self.NFT_position_contract.get_function_by_signature("increaseLiquidity(tuple)")
        function = increase_liquidity(increase_liquidity_params)
        
        return self._build_and_send_tx(function)
    
    def decrease_liquidity_v3(
        self, tokenId: int, liquidity: int, amount0Min: int, amount1Min: int
    ) -> HexBytes:
        print("Decrease Liquidity")
        ...
        decrease_liquidity = self.NFT_position_contract.get_function_by_signature("decreaseLiquidity(tuple)")
        function = decrease_liquidity(decrease_liquidity_params)
        
        return self._build_and_send_tx(function)
    
    def callback(a1: int, a2: int):
    	print(a1)

    # ------ Approval Utils ------------------------------------------------------------
    def approve_to_pool(self, token: AddressLike, amount: int, pool_address: AddressLike) -> None:
        """Give an pool approval of a token."""
        ...
        time.sleep(1)    
        print(f"Approved {_addr_to_str(token)}")
    ...
    def add_liquidity(
        ...
        """Add liquidity to the pool."""
        if self.version == 1:
            return self.add_liquidity_v1(token, max_eth, min_liquidity)
        #if self.version == 3:
        #    return self.add_liquidity_v3(token, max_eth, min_liquidity)
    ...
    def _is_approved(self, token: AddressLike) -> bool:
        ...
        print(amount)
        print(self.max_approval_check_int)

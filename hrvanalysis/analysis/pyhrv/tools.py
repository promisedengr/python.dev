...
def plot_ecg(signal=None,
	...
	# Compute time vector
	if t is None:
		t = pu.time_vector(signal, sampling_rate=sampling_rate)

	# Configure interval of visualized signal
	if interval == 'complete':
		interval = [0, t[-1]]
	else:
		interval = pu.check_interval(interval, limits=[0, t[-1]], default=[0, 10])
...
def heart_rate(nni=None, rpeaks=None):
	...
	if nni is None and rpeaks is not None:
		...
		if type(nni) is list or type(nni) is np.ndarray:
			nni = pu.nn_format(nni) if len(nni) > 1 else nni[0]
...
def heart_rate_heatplot(nni=None,
	...
	t = np.cumsum(nn) / 1000
	interval = pu.check_interval(interval, limits=[0, t[-1]], default=[0, t[-1]])
...
def time_varying(nni=None, rpeaks=None, parameter='sdnn', window='n20', interpolation=None, show=True, mode='normal'):
	...
	elif parameter not in pu.load_hrv_keys_json().keys():
	...
	hrv_keys = pu.load_hrv_keys_json()
...
def radar_chart(nni=None,
	...
	para_func = pu.load_hrv_keys_json()
	...
	if comparison_nni is not None and comparison_rpeaks is not None:
		...
		comp_nn = pu.check_input(comparison_nni, comparison_rpeaks)
	...
	register_projection(pu.pyHRVRadarAxes)
...
def hrv_export(results=None, path=None, efile=None, comment=None, plots=False):
	...
	efile, _ = pu.check_fname(path, 'json', efile)
